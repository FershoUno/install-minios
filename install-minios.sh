...
force_exit() {
    yad --info --title="$INFO" --text="$MESSAGE_TEXT_FORCE_EXIT" --no-wrap
    exit 0
}
...
english_install() {
    ...
    TEXT_FORMAT_60_70="# Mounting disk to /mnt/${disk}1..."
...
language_select() {
    ...
    language=$(yad --list --title="MiniOS Installer" --text="Select the installer language:" --column="Language" "English" "Español" "Русский" --height=200 --width=250 | cut -d"|" -f1)
...
show_main_menu() {
    CHOICE=$(yad --list --title="$MAIN_MENU" --column="$MAIN_OPTION_COLUMN" --text="$MAIN_TEXT" \
        "$MAIN_OPTION_ONE" "$EXIT" | cut -d"|" -f1)
...
select_disk_and_install() {
    ...
    SELECTED_DISK=$(yad --list --title="$SELECT_DISK_TITLE" --column="$COLUMN_DISK" --text="$SELECT_DISK_TEXT" ${DISKS} | cut -d"|" -f1)

    if [ -z "$SELECTED_DISK" ]; then
        show_main_menu
    else

        yad --question --title="$TITLE_WARNING" --text="$TEXT_WARNING1 /dev/$SELECTED_DISK $TEXT_WARNING2" --no-wrap
